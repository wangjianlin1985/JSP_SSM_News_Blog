...
import com.yuanma.programmer.entity.admin.News;
import com.yuanma.programmer.entity.admin.NewsCategory;
import com.yuanma.programmer.page.admin.Page;
import com.yuanma.programmer.service.admin.CommentService;
import com.yuanma.programmer.service.admin.NewsCategoryService;
import com.yuanma.programmer.service.admin.NewsService;
...
public class HomeNewsController {
	 ...
	 * @return
	 */
	@RequestMapping(value="/get_recommed_list",method=RequestMethod.GET)
	@ResponseBody
	public Map<String, Object> getRecommedList(Page page,
			@RequestParam(name="id") Long id
			){
		Map<String, Object> ret = new HashMap<String, Object>();
		List<News> returNews = new ArrayList<News>();
		if(id != -1) {
			News news = newsService.find(id);
			...
			List<News> newsList = newsService.findList(queryMap);
			for(String tag:tags) {
				for(News temp:newsList) {
					if(temp.getTags().contains(tag)&&temp.getId()!=id) {
						returNews.add(temp);
						if(returNews.size() == 5) {
							break;
						}
			...
			returNews = newsService.findMostViewList(5);
		}
		if(returNews.size()<5) {
			List<News> subList = newsService.findLastCommentList(5-returNews.size());
			returNews.addAll(subList);
		}
		
		ret.put("type", "success");
		ret.put("newsList", returNews);
		return ret;
	}
